...
if ! wget -q -O "/tmp/$TARBALL" "$DOWNLOAD_URL" 2>> "$LOG" ; then
...
if ! wget -q -O "/tmp/checksums.txt" "$CHECKSUM_URL" 2>> "$LOG" ; then
...
if $SUDO systemctl is-active --quiet AdGuardHome.service; then
	$SUDO systemctl stop AdGuardHome.service 2>> "$LOG" || { echo "Error: Failed to stop AdGuardHome service." >&2; exit 1; }
	else echo "AdGuardHome service is not running."
fi



if ! $SUDO cp /opt/AdGuardHome/AdGuardHome.yaml "$BACKUP_DIR" 2>> "$LOG" ; then
	echo "Error: Failed to create backup of AdGuardHome configuration file." >&2
	exit 1
fi

if ! $SUDO cp -r /opt/AdGuardHome/data "$BACKUP_DIR" 2>> "$LOG" ; then
...
if ! tar -zxvf "/tmp/$TARBALL" -C /tmp/ 2>> "$LOG" ; then
	echo "Error: Failed to extract AdGuardHome tarball." >&2
	exit 1
fi

if ! $SUDO cp /tmp/AdGuardHome/AdGuardHome /opt/AdGuardHome/ 2>> "$LOG" ; then
...
if ! $SUDO systemctl start AdGuardHome.service 2>> "$LOG" ; then
